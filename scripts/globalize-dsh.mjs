...
//
// 策略（避开管理员权限）：
//   - 目标前缀优先取 npm 全局前缀；若不可写（如默认 /usr/local），
//     Windows 退回 %APPDATA%\npm（npm 官方用户级前缀，通常已在 PATH），
//     macOS/Linux 退回 ~/.npm-global（自建并写入 shell rc）。
...
import { accessSync, constants, mkdirSync, readFileSync, appendFileSync, existsSync } from 'node:fs'
...
let prefix = ''
try {
  const p = spawnSync('npm', ['prefix', '-g'], { encoding: 'utf8' })
  if (p.status === 0) prefix = p.stdout.trim()
} catch { /* ignore */ }
if (!prefix || !writable(prefix)) {
  prefix = process.platform === 'win32'
    ? join(process.env.APPDATA ?? homedir(), 'npm')
    : join(homedir(), '.npm-global')
  mkdirSync(prefix, { recursive: true })
  console.log(`[globalize-dsh] npm 全局前缀不可写，改用用户级前缀：${prefix}`)
}
...
const v = spawnSync(shim, ['--version'], { encoding: 'utf8', shell: process.platform === 'win32' })
