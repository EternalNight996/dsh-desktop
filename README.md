...
| 装 Node | 要 | 免 | **免** |
| 自动启动 dsh | ❌ 手动 | ✅ | ✅ |
| dsh 引擎更新 | 手动 npm | 手动 | **✅ 在线后台检查 + 多版本下拉一键升级；离线 `just update-offline` 一键** |
...
「更新设置」入口由内置 dsh 客户端插件经官方槽位挂载，双处常驻：**DSH 侧边栏底部**（工作区左下角）与**设置页左侧栏底部**（设置 → 左侧栏最下「更新设置」）。点开即弹出独立**设置·更新设置窗口**：内置桌面壳与 dsh 双更新入口、版本与运行方式一览、随时一键升级。关闭主窗口会最小化到托盘，更新设置常驻托盘与设置窗口。
...
- **v0.7.0**：**桌面壳更新升级为多版本下拉**（列 GitHub/Gitee 全部发布版本，任选下载安装）+ **Gitee/GitHub 双源支持**（版本列表 Gitee tags 优先、下载 Gitee/GitHub 双源兜底，国内网络可选），下载后 **minisign 签名校验通过才安装**；**dsh 安装进度条实时化**（改 `--loglevel=http` 解析 npm http fetch 行，实时显示已下载包数）
- **v0.6.1**：dsh 更新升级为**多版本下拉选择**——设置窗口一键拉取 npm 官方全部版本（绿=正式 / 黄=预发布 / 蓝=当前已装），任选版本安装 + 一键到 latest tag + npm 安装进度实时显示；**适配 dsh alpha 的 token 认证**（Web UI 根路径带 token 才可进，桌面壳自动解析并导航，修复升级预发布版后卡加载页）；升级流程改为**先停 dsh 再安装**（防运行时锁住原生 `.node` 文件导致 EBUSY 半装损坏）+ 失败回滚重拉
